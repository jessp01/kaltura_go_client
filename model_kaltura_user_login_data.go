/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// KalturaUserLoginData struct for KalturaUserLoginData
type KalturaUserLoginData struct {
	Id *string `json:"id,omitempty"`
	LoginEmail *string `json:"loginEmail,omitempty"`
}

// NewKalturaUserLoginData instantiates a new KalturaUserLoginData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewKalturaUserLoginData() *KalturaUserLoginData {
	this := KalturaUserLoginData{}
	return &this
}

// NewKalturaUserLoginDataWithDefaults instantiates a new KalturaUserLoginData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewKalturaUserLoginDataWithDefaults() *KalturaUserLoginData {
	this := KalturaUserLoginData{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *KalturaUserLoginData) GetId() string {
	if o == nil || o.Id == nil {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *KalturaUserLoginData) GetIdOk() (*string, bool) {
	if o == nil || o.Id == nil {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *KalturaUserLoginData) HasId() bool {
	if o != nil && o.Id != nil {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *KalturaUserLoginData) SetId(v string) {
	o.Id = &v
}

// GetLoginEmail returns the LoginEmail field value if set, zero value otherwise.
func (o *KalturaUserLoginData) GetLoginEmail() string {
	if o == nil || o.LoginEmail == nil {
		var ret string
		return ret
	}
	return *o.LoginEmail
}

// GetLoginEmailOk returns a tuple with the LoginEmail field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *KalturaUserLoginData) GetLoginEmailOk() (*string, bool) {
	if o == nil || o.LoginEmail == nil {
		return nil, false
	}
	return o.LoginEmail, true
}

// HasLoginEmail returns a boolean if a field has been set.
func (o *KalturaUserLoginData) HasLoginEmail() bool {
	if o != nil && o.LoginEmail != nil {
		return true
	}

	return false
}

// SetLoginEmail gets a reference to the given string and assigns it to the LoginEmail field.
func (o *KalturaUserLoginData) SetLoginEmail(v string) {
	o.LoginEmail = &v
}

func (o KalturaUserLoginData) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Id != nil {
		toSerialize["id"] = o.Id
	}
	if o.LoginEmail != nil {
		toSerialize["loginEmail"] = o.LoginEmail
	}
	return json.Marshal(toSerialize)
}

type NullableKalturaUserLoginData struct {
	value *KalturaUserLoginData
	isSet bool
}

func (v NullableKalturaUserLoginData) Get() *KalturaUserLoginData {
	return v.value
}

func (v *NullableKalturaUserLoginData) Set(val *KalturaUserLoginData) {
	v.value = val
	v.isSet = true
}

func (v NullableKalturaUserLoginData) IsSet() bool {
	return v.isSet
}

func (v *NullableKalturaUserLoginData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableKalturaUserLoginData(val *KalturaUserLoginData) *NullableKalturaUserLoginData {
	return &NullableKalturaUserLoginData{value: val, isSet: true}
}

func (v NullableKalturaUserLoginData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableKalturaUserLoginData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


