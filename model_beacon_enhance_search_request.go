/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// BeaconEnhanceSearchRequest struct for BeaconEnhanceSearchRequest
type BeaconEnhanceSearchRequest struct {
	Filter *KalturaBeaconEnhanceFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewBeaconEnhanceSearchRequest instantiates a new BeaconEnhanceSearchRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBeaconEnhanceSearchRequest() *BeaconEnhanceSearchRequest {
	this := BeaconEnhanceSearchRequest{}
	return &this
}

// NewBeaconEnhanceSearchRequestWithDefaults instantiates a new BeaconEnhanceSearchRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBeaconEnhanceSearchRequestWithDefaults() *BeaconEnhanceSearchRequest {
	this := BeaconEnhanceSearchRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *BeaconEnhanceSearchRequest) GetFilter() KalturaBeaconEnhanceFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaBeaconEnhanceFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BeaconEnhanceSearchRequest) GetFilterOk() (*KalturaBeaconEnhanceFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *BeaconEnhanceSearchRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaBeaconEnhanceFilter and assigns it to the Filter field.
func (o *BeaconEnhanceSearchRequest) SetFilter(v KalturaBeaconEnhanceFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *BeaconEnhanceSearchRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BeaconEnhanceSearchRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *BeaconEnhanceSearchRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *BeaconEnhanceSearchRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o BeaconEnhanceSearchRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableBeaconEnhanceSearchRequest struct {
	value *BeaconEnhanceSearchRequest
	isSet bool
}

func (v NullableBeaconEnhanceSearchRequest) Get() *BeaconEnhanceSearchRequest {
	return v.value
}

func (v *NullableBeaconEnhanceSearchRequest) Set(val *BeaconEnhanceSearchRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableBeaconEnhanceSearchRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableBeaconEnhanceSearchRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBeaconEnhanceSearchRequest(val *BeaconEnhanceSearchRequest) *NullableBeaconEnhanceSearchRequest {
	return &NullableBeaconEnhanceSearchRequest{value: val, isSet: true}
}

func (v NullableBeaconEnhanceSearchRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBeaconEnhanceSearchRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


