/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// JobsListBatchJobsRequest struct for JobsListBatchJobsRequest
type JobsListBatchJobsRequest struct {
	Filter *KalturaBatchJobFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewJobsListBatchJobsRequest instantiates a new JobsListBatchJobsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewJobsListBatchJobsRequest() *JobsListBatchJobsRequest {
	this := JobsListBatchJobsRequest{}
	return &this
}

// NewJobsListBatchJobsRequestWithDefaults instantiates a new JobsListBatchJobsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewJobsListBatchJobsRequestWithDefaults() *JobsListBatchJobsRequest {
	this := JobsListBatchJobsRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *JobsListBatchJobsRequest) GetFilter() KalturaBatchJobFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaBatchJobFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JobsListBatchJobsRequest) GetFilterOk() (*KalturaBatchJobFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *JobsListBatchJobsRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaBatchJobFilter and assigns it to the Filter field.
func (o *JobsListBatchJobsRequest) SetFilter(v KalturaBatchJobFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *JobsListBatchJobsRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JobsListBatchJobsRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *JobsListBatchJobsRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *JobsListBatchJobsRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o JobsListBatchJobsRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableJobsListBatchJobsRequest struct {
	value *JobsListBatchJobsRequest
	isSet bool
}

func (v NullableJobsListBatchJobsRequest) Get() *JobsListBatchJobsRequest {
	return v.value
}

func (v *NullableJobsListBatchJobsRequest) Set(val *JobsListBatchJobsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableJobsListBatchJobsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableJobsListBatchJobsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableJobsListBatchJobsRequest(val *JobsListBatchJobsRequest) *NullableJobsListBatchJobsRequest {
	return &NullableJobsListBatchJobsRequest{value: val, isSet: true}
}

func (v NullableJobsListBatchJobsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableJobsListBatchJobsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


