/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// AppTokenListRequest struct for AppTokenListRequest
type AppTokenListRequest struct {
	Filter *KalturaAppTokenFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewAppTokenListRequest instantiates a new AppTokenListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAppTokenListRequest() *AppTokenListRequest {
	this := AppTokenListRequest{}
	return &this
}

// NewAppTokenListRequestWithDefaults instantiates a new AppTokenListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAppTokenListRequestWithDefaults() *AppTokenListRequest {
	this := AppTokenListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *AppTokenListRequest) GetFilter() KalturaAppTokenFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaAppTokenFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AppTokenListRequest) GetFilterOk() (*KalturaAppTokenFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *AppTokenListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaAppTokenFilter and assigns it to the Filter field.
func (o *AppTokenListRequest) SetFilter(v KalturaAppTokenFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *AppTokenListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AppTokenListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *AppTokenListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *AppTokenListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o AppTokenListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableAppTokenListRequest struct {
	value *AppTokenListRequest
	isSet bool
}

func (v NullableAppTokenListRequest) Get() *AppTokenListRequest {
	return v.value
}

func (v *NullableAppTokenListRequest) Set(val *AppTokenListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableAppTokenListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableAppTokenListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAppTokenListRequest(val *AppTokenListRequest) *NullableAppTokenListRequest {
	return &NullableAppTokenListRequest{value: val, isSet: true}
}

func (v NullableAppTokenListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAppTokenListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


