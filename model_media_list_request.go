/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// MediaListRequest struct for MediaListRequest
type MediaListRequest struct {
	Filter *KalturaMediaEntryFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewMediaListRequest instantiates a new MediaListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMediaListRequest() *MediaListRequest {
	this := MediaListRequest{}
	return &this
}

// NewMediaListRequestWithDefaults instantiates a new MediaListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMediaListRequestWithDefaults() *MediaListRequest {
	this := MediaListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *MediaListRequest) GetFilter() KalturaMediaEntryFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaMediaEntryFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaListRequest) GetFilterOk() (*KalturaMediaEntryFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *MediaListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaMediaEntryFilter and assigns it to the Filter field.
func (o *MediaListRequest) SetFilter(v KalturaMediaEntryFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *MediaListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MediaListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *MediaListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *MediaListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o MediaListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableMediaListRequest struct {
	value *MediaListRequest
	isSet bool
}

func (v NullableMediaListRequest) Get() *MediaListRequest {
	return v.value
}

func (v *NullableMediaListRequest) Set(val *MediaListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableMediaListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableMediaListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMediaListRequest(val *MediaListRequest) *NullableMediaListRequest {
	return &NullableMediaListRequest{value: val, isSet: true}
}

func (v NullableMediaListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMediaListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


