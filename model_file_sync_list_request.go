/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// FileSyncListRequest struct for FileSyncListRequest
type FileSyncListRequest struct {
	Filter *KalturaFileSyncFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewFileSyncListRequest instantiates a new FileSyncListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFileSyncListRequest() *FileSyncListRequest {
	this := FileSyncListRequest{}
	return &this
}

// NewFileSyncListRequestWithDefaults instantiates a new FileSyncListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFileSyncListRequestWithDefaults() *FileSyncListRequest {
	this := FileSyncListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *FileSyncListRequest) GetFilter() KalturaFileSyncFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaFileSyncFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FileSyncListRequest) GetFilterOk() (*KalturaFileSyncFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *FileSyncListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaFileSyncFilter and assigns it to the Filter field.
func (o *FileSyncListRequest) SetFilter(v KalturaFileSyncFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *FileSyncListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FileSyncListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *FileSyncListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *FileSyncListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o FileSyncListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableFileSyncListRequest struct {
	value *FileSyncListRequest
	isSet bool
}

func (v NullableFileSyncListRequest) Get() *FileSyncListRequest {
	return v.value
}

func (v *NullableFileSyncListRequest) Set(val *FileSyncListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableFileSyncListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableFileSyncListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFileSyncListRequest(val *FileSyncListRequest) *NullableFileSyncListRequest {
	return &NullableFileSyncListRequest{value: val, isSet: true}
}

func (v NullableFileSyncListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFileSyncListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


