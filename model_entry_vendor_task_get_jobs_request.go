/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// EntryVendorTaskGetJobsRequest struct for EntryVendorTaskGetJobsRequest
type EntryVendorTaskGetJobsRequest struct {
	Filter *KalturaEntryVendorTaskFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewEntryVendorTaskGetJobsRequest instantiates a new EntryVendorTaskGetJobsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewEntryVendorTaskGetJobsRequest() *EntryVendorTaskGetJobsRequest {
	this := EntryVendorTaskGetJobsRequest{}
	return &this
}

// NewEntryVendorTaskGetJobsRequestWithDefaults instantiates a new EntryVendorTaskGetJobsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewEntryVendorTaskGetJobsRequestWithDefaults() *EntryVendorTaskGetJobsRequest {
	this := EntryVendorTaskGetJobsRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *EntryVendorTaskGetJobsRequest) GetFilter() KalturaEntryVendorTaskFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaEntryVendorTaskFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *EntryVendorTaskGetJobsRequest) GetFilterOk() (*KalturaEntryVendorTaskFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *EntryVendorTaskGetJobsRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaEntryVendorTaskFilter and assigns it to the Filter field.
func (o *EntryVendorTaskGetJobsRequest) SetFilter(v KalturaEntryVendorTaskFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *EntryVendorTaskGetJobsRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *EntryVendorTaskGetJobsRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *EntryVendorTaskGetJobsRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *EntryVendorTaskGetJobsRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o EntryVendorTaskGetJobsRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableEntryVendorTaskGetJobsRequest struct {
	value *EntryVendorTaskGetJobsRequest
	isSet bool
}

func (v NullableEntryVendorTaskGetJobsRequest) Get() *EntryVendorTaskGetJobsRequest {
	return v.value
}

func (v *NullableEntryVendorTaskGetJobsRequest) Set(val *EntryVendorTaskGetJobsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableEntryVendorTaskGetJobsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableEntryVendorTaskGetJobsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableEntryVendorTaskGetJobsRequest(val *EntryVendorTaskGetJobsRequest) *NullableEntryVendorTaskGetJobsRequest {
	return &NullableEntryVendorTaskGetJobsRequest{value: val, isSet: true}
}

func (v NullableEntryVendorTaskGetJobsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableEntryVendorTaskGetJobsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


