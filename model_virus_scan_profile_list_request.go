/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// VirusScanProfileListRequest struct for VirusScanProfileListRequest
type VirusScanProfileListRequest struct {
	Filter *KalturaVirusScanProfileFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewVirusScanProfileListRequest instantiates a new VirusScanProfileListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewVirusScanProfileListRequest() *VirusScanProfileListRequest {
	this := VirusScanProfileListRequest{}
	return &this
}

// NewVirusScanProfileListRequestWithDefaults instantiates a new VirusScanProfileListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewVirusScanProfileListRequestWithDefaults() *VirusScanProfileListRequest {
	this := VirusScanProfileListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *VirusScanProfileListRequest) GetFilter() KalturaVirusScanProfileFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaVirusScanProfileFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VirusScanProfileListRequest) GetFilterOk() (*KalturaVirusScanProfileFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *VirusScanProfileListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaVirusScanProfileFilter and assigns it to the Filter field.
func (o *VirusScanProfileListRequest) SetFilter(v KalturaVirusScanProfileFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *VirusScanProfileListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VirusScanProfileListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *VirusScanProfileListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *VirusScanProfileListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o VirusScanProfileListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableVirusScanProfileListRequest struct {
	value *VirusScanProfileListRequest
	isSet bool
}

func (v NullableVirusScanProfileListRequest) Get() *VirusScanProfileListRequest {
	return v.value
}

func (v *NullableVirusScanProfileListRequest) Set(val *VirusScanProfileListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableVirusScanProfileListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableVirusScanProfileListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVirusScanProfileListRequest(val *VirusScanProfileListRequest) *NullableVirusScanProfileListRequest {
	return &NullableVirusScanProfileListRequest{value: val, isSet: true}
}

func (v NullableVirusScanProfileListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVirusScanProfileListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


