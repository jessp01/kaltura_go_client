/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// BaseEntryListRequest struct for BaseEntryListRequest
type BaseEntryListRequest struct {
	Filter *KalturaBaseEntryFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewBaseEntryListRequest instantiates a new BaseEntryListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBaseEntryListRequest() *BaseEntryListRequest {
	this := BaseEntryListRequest{}
	return &this
}

// NewBaseEntryListRequestWithDefaults instantiates a new BaseEntryListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBaseEntryListRequestWithDefaults() *BaseEntryListRequest {
	this := BaseEntryListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *BaseEntryListRequest) GetFilter() KalturaBaseEntryFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaBaseEntryFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BaseEntryListRequest) GetFilterOk() (*KalturaBaseEntryFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *BaseEntryListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaBaseEntryFilter and assigns it to the Filter field.
func (o *BaseEntryListRequest) SetFilter(v KalturaBaseEntryFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *BaseEntryListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BaseEntryListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *BaseEntryListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *BaseEntryListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o BaseEntryListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableBaseEntryListRequest struct {
	value *BaseEntryListRequest
	isSet bool
}

func (v NullableBaseEntryListRequest) Get() *BaseEntryListRequest {
	return v.value
}

func (v *NullableBaseEntryListRequest) Set(val *BaseEntryListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableBaseEntryListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableBaseEntryListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBaseEntryListRequest(val *BaseEntryListRequest) *NullableBaseEntryListRequest {
	return &NullableBaseEntryListRequest{value: val, isSet: true}
}

func (v NullableBaseEntryListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBaseEntryListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


