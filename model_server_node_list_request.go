/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// ServerNodeListRequest struct for ServerNodeListRequest
type ServerNodeListRequest struct {
	Filter *KalturaServerNodeFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewServerNodeListRequest instantiates a new ServerNodeListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewServerNodeListRequest() *ServerNodeListRequest {
	this := ServerNodeListRequest{}
	return &this
}

// NewServerNodeListRequestWithDefaults instantiates a new ServerNodeListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewServerNodeListRequestWithDefaults() *ServerNodeListRequest {
	this := ServerNodeListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *ServerNodeListRequest) GetFilter() KalturaServerNodeFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaServerNodeFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServerNodeListRequest) GetFilterOk() (*KalturaServerNodeFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *ServerNodeListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaServerNodeFilter and assigns it to the Filter field.
func (o *ServerNodeListRequest) SetFilter(v KalturaServerNodeFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *ServerNodeListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServerNodeListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *ServerNodeListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *ServerNodeListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o ServerNodeListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableServerNodeListRequest struct {
	value *ServerNodeListRequest
	isSet bool
}

func (v NullableServerNodeListRequest) Get() *ServerNodeListRequest {
	return v.value
}

func (v *NullableServerNodeListRequest) Set(val *ServerNodeListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableServerNodeListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableServerNodeListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableServerNodeListRequest(val *ServerNodeListRequest) *NullableServerNodeListRequest {
	return &NullableServerNodeListRequest{value: val, isSet: true}
}

func (v NullableServerNodeListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableServerNodeListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


