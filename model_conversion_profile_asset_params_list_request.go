/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// ConversionProfileAssetParamsListRequest struct for ConversionProfileAssetParamsListRequest
type ConversionProfileAssetParamsListRequest struct {
	Filter *KalturaConversionProfileAssetParamsFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewConversionProfileAssetParamsListRequest instantiates a new ConversionProfileAssetParamsListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConversionProfileAssetParamsListRequest() *ConversionProfileAssetParamsListRequest {
	this := ConversionProfileAssetParamsListRequest{}
	return &this
}

// NewConversionProfileAssetParamsListRequestWithDefaults instantiates a new ConversionProfileAssetParamsListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConversionProfileAssetParamsListRequestWithDefaults() *ConversionProfileAssetParamsListRequest {
	this := ConversionProfileAssetParamsListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *ConversionProfileAssetParamsListRequest) GetFilter() KalturaConversionProfileAssetParamsFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaConversionProfileAssetParamsFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConversionProfileAssetParamsListRequest) GetFilterOk() (*KalturaConversionProfileAssetParamsFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *ConversionProfileAssetParamsListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaConversionProfileAssetParamsFilter and assigns it to the Filter field.
func (o *ConversionProfileAssetParamsListRequest) SetFilter(v KalturaConversionProfileAssetParamsFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *ConversionProfileAssetParamsListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConversionProfileAssetParamsListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *ConversionProfileAssetParamsListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *ConversionProfileAssetParamsListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o ConversionProfileAssetParamsListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableConversionProfileAssetParamsListRequest struct {
	value *ConversionProfileAssetParamsListRequest
	isSet bool
}

func (v NullableConversionProfileAssetParamsListRequest) Get() *ConversionProfileAssetParamsListRequest {
	return v.value
}

func (v *NullableConversionProfileAssetParamsListRequest) Set(val *ConversionProfileAssetParamsListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableConversionProfileAssetParamsListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableConversionProfileAssetParamsListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConversionProfileAssetParamsListRequest(val *ConversionProfileAssetParamsListRequest) *NullableConversionProfileAssetParamsListRequest {
	return &NullableConversionProfileAssetParamsListRequest{value: val, isSet: true}
}

func (v NullableConversionProfileAssetParamsListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConversionProfileAssetParamsListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


