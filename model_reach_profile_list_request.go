/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// ReachProfileListRequest struct for ReachProfileListRequest
type ReachProfileListRequest struct {
	Filter *KalturaReachProfileFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewReachProfileListRequest instantiates a new ReachProfileListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewReachProfileListRequest() *ReachProfileListRequest {
	this := ReachProfileListRequest{}
	return &this
}

// NewReachProfileListRequestWithDefaults instantiates a new ReachProfileListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewReachProfileListRequestWithDefaults() *ReachProfileListRequest {
	this := ReachProfileListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *ReachProfileListRequest) GetFilter() KalturaReachProfileFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaReachProfileFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ReachProfileListRequest) GetFilterOk() (*KalturaReachProfileFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *ReachProfileListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaReachProfileFilter and assigns it to the Filter field.
func (o *ReachProfileListRequest) SetFilter(v KalturaReachProfileFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *ReachProfileListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ReachProfileListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *ReachProfileListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *ReachProfileListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o ReachProfileListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableReachProfileListRequest struct {
	value *ReachProfileListRequest
	isSet bool
}

func (v NullableReachProfileListRequest) Get() *ReachProfileListRequest {
	return v.value
}

func (v *NullableReachProfileListRequest) Set(val *ReachProfileListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableReachProfileListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableReachProfileListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableReachProfileListRequest(val *ReachProfileListRequest) *NullableReachProfileListRequest {
	return &NullableReachProfileListRequest{value: val, isSet: true}
}

func (v NullableReachProfileListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableReachProfileListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


