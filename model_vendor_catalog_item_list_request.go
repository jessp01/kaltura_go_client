/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// VendorCatalogItemListRequest struct for VendorCatalogItemListRequest
type VendorCatalogItemListRequest struct {
	Filter *KalturaVendorCatalogItemFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewVendorCatalogItemListRequest instantiates a new VendorCatalogItemListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewVendorCatalogItemListRequest() *VendorCatalogItemListRequest {
	this := VendorCatalogItemListRequest{}
	return &this
}

// NewVendorCatalogItemListRequestWithDefaults instantiates a new VendorCatalogItemListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewVendorCatalogItemListRequestWithDefaults() *VendorCatalogItemListRequest {
	this := VendorCatalogItemListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *VendorCatalogItemListRequest) GetFilter() KalturaVendorCatalogItemFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaVendorCatalogItemFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VendorCatalogItemListRequest) GetFilterOk() (*KalturaVendorCatalogItemFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *VendorCatalogItemListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaVendorCatalogItemFilter and assigns it to the Filter field.
func (o *VendorCatalogItemListRequest) SetFilter(v KalturaVendorCatalogItemFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *VendorCatalogItemListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VendorCatalogItemListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *VendorCatalogItemListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *VendorCatalogItemListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o VendorCatalogItemListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableVendorCatalogItemListRequest struct {
	value *VendorCatalogItemListRequest
	isSet bool
}

func (v NullableVendorCatalogItemListRequest) Get() *VendorCatalogItemListRequest {
	return v.value
}

func (v *NullableVendorCatalogItemListRequest) Set(val *VendorCatalogItemListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableVendorCatalogItemListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableVendorCatalogItemListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVendorCatalogItemListRequest(val *VendorCatalogItemListRequest) *NullableVendorCatalogItemListRequest {
	return &NullableVendorCatalogItemListRequest{value: val, isSet: true}
}

func (v NullableVendorCatalogItemListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVendorCatalogItemListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


