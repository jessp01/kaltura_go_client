/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// BatchcontrolListCommandsRequest struct for BatchcontrolListCommandsRequest
type BatchcontrolListCommandsRequest struct {
	Filter *KalturaControlPanelCommandFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewBatchcontrolListCommandsRequest instantiates a new BatchcontrolListCommandsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBatchcontrolListCommandsRequest() *BatchcontrolListCommandsRequest {
	this := BatchcontrolListCommandsRequest{}
	return &this
}

// NewBatchcontrolListCommandsRequestWithDefaults instantiates a new BatchcontrolListCommandsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBatchcontrolListCommandsRequestWithDefaults() *BatchcontrolListCommandsRequest {
	this := BatchcontrolListCommandsRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *BatchcontrolListCommandsRequest) GetFilter() KalturaControlPanelCommandFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaControlPanelCommandFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BatchcontrolListCommandsRequest) GetFilterOk() (*KalturaControlPanelCommandFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *BatchcontrolListCommandsRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaControlPanelCommandFilter and assigns it to the Filter field.
func (o *BatchcontrolListCommandsRequest) SetFilter(v KalturaControlPanelCommandFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *BatchcontrolListCommandsRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BatchcontrolListCommandsRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *BatchcontrolListCommandsRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *BatchcontrolListCommandsRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o BatchcontrolListCommandsRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableBatchcontrolListCommandsRequest struct {
	value *BatchcontrolListCommandsRequest
	isSet bool
}

func (v NullableBatchcontrolListCommandsRequest) Get() *BatchcontrolListCommandsRequest {
	return v.value
}

func (v *NullableBatchcontrolListCommandsRequest) Set(val *BatchcontrolListCommandsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableBatchcontrolListCommandsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableBatchcontrolListCommandsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBatchcontrolListCommandsRequest(val *BatchcontrolListCommandsRequest) *NullableBatchcontrolListCommandsRequest {
	return &NullableBatchcontrolListCommandsRequest{value: val, isSet: true}
}

func (v NullableBatchcontrolListCommandsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBatchcontrolListCommandsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


