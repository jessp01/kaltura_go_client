/*
Kaltura VPaaS

The Kaltura VPaaS API

API version: 18.12.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// ShortLinkListRequest struct for ShortLinkListRequest
type ShortLinkListRequest struct {
	Filter *KalturaShortLinkFilter `json:"filter,omitempty"`
	Pager *KalturaFilterPager `json:"pager,omitempty"`
}

// NewShortLinkListRequest instantiates a new ShortLinkListRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewShortLinkListRequest() *ShortLinkListRequest {
	this := ShortLinkListRequest{}
	return &this
}

// NewShortLinkListRequestWithDefaults instantiates a new ShortLinkListRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewShortLinkListRequestWithDefaults() *ShortLinkListRequest {
	this := ShortLinkListRequest{}
	return &this
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *ShortLinkListRequest) GetFilter() KalturaShortLinkFilter {
	if o == nil || o.Filter == nil {
		var ret KalturaShortLinkFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ShortLinkListRequest) GetFilterOk() (*KalturaShortLinkFilter, bool) {
	if o == nil || o.Filter == nil {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *ShortLinkListRequest) HasFilter() bool {
	if o != nil && o.Filter != nil {
		return true
	}

	return false
}

// SetFilter gets a reference to the given KalturaShortLinkFilter and assigns it to the Filter field.
func (o *ShortLinkListRequest) SetFilter(v KalturaShortLinkFilter) {
	o.Filter = &v
}

// GetPager returns the Pager field value if set, zero value otherwise.
func (o *ShortLinkListRequest) GetPager() KalturaFilterPager {
	if o == nil || o.Pager == nil {
		var ret KalturaFilterPager
		return ret
	}
	return *o.Pager
}

// GetPagerOk returns a tuple with the Pager field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ShortLinkListRequest) GetPagerOk() (*KalturaFilterPager, bool) {
	if o == nil || o.Pager == nil {
		return nil, false
	}
	return o.Pager, true
}

// HasPager returns a boolean if a field has been set.
func (o *ShortLinkListRequest) HasPager() bool {
	if o != nil && o.Pager != nil {
		return true
	}

	return false
}

// SetPager gets a reference to the given KalturaFilterPager and assigns it to the Pager field.
func (o *ShortLinkListRequest) SetPager(v KalturaFilterPager) {
	o.Pager = &v
}

func (o ShortLinkListRequest) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Filter != nil {
		toSerialize["filter"] = o.Filter
	}
	if o.Pager != nil {
		toSerialize["pager"] = o.Pager
	}
	return json.Marshal(toSerialize)
}

type NullableShortLinkListRequest struct {
	value *ShortLinkListRequest
	isSet bool
}

func (v NullableShortLinkListRequest) Get() *ShortLinkListRequest {
	return v.value
}

func (v *NullableShortLinkListRequest) Set(val *ShortLinkListRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableShortLinkListRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableShortLinkListRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableShortLinkListRequest(val *ShortLinkListRequest) *NullableShortLinkListRequest {
	return &NullableShortLinkListRequest{value: val, isSet: true}
}

func (v NullableShortLinkListRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableShortLinkListRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


